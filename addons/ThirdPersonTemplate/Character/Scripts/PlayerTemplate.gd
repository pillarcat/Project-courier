...
#	Jump input and Mechanics
	...
	if Input.is_action_just_pressed("jump") and ((is_attacking !=true) and (is_rolling !=true)) and can_double_jump and !is_on_floor():
		vertical_velocity = Vector3.UP * jump_force
		can_double_jump= false
		$DoubleJump.play()
		playback.start(jump_node_name)
		
		
	if Input.is_action_just_pressed("Dash") and !is_on_floor() and can_dash:
		horizontal_velocity = direction * dash_speed
		vertical_velocity = Vector3.DOWN * 0
		can_dash = false
		$Dash.play()
		playback.start(dash_node_name)
